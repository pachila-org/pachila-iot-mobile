...
public final class ViewfinderView extends View {
	...
	public ViewfinderView(Context context, AttributeSet attrs) {
		...
		laserColor = resources.getColor(R.color.line_color);
	...
	public void onDraw(Canvas canvas) {
		...
		//这里画取景框四个角落的绿色夹角
		paintCorner.setColor(laserColor);
		paintCorner.setAntiAlias(true);
		paintCorner.setStrokeWidth(5);
		int lineWidth = frame.left / 4 ;
		canvas.drawLine(frame.left-2.5f, frame.top, frame.left + lineWidth, frame.top, paintCorner);
		canvas.drawLine(frame.left, frame.top, frame.left, frame.top+lineWidth, paintCorner);
		canvas.drawLine(frame.right-lineWidth, frame.top, frame.right+2.5f, frame.top, paintCorner);
		canvas.drawLine(frame.right, frame.top, frame.right, frame.top+lineWidth, paintCorner);
		canvas.drawLine(frame.left-2.5f, frame.bottom, frame.left + lineWidth, frame.bottom, paintCorner);
		canvas.drawLine(frame.left, frame.bottom-lineWidth, frame.left, frame.bottom, paintCorner);
		canvas.drawLine(frame.right-lineWidth, frame.bottom, frame.right+2.5f, frame.bottom, paintCorner);
		canvas.drawLine(frame.right, frame.bottom-lineWidth, frame.right, frame.bottom, paintCorner);
		...
		if (resultBitmap != null) {
			...
			// Draw a r "laser scanner" line through the middle to show
			// decoding is active
        ...
        canvas.drawText(text, (width - textWidth)/2, (float) (frame.bottom + (float)30 *density), paint); 
